...
<!-- File: resources/views/admin/ujians/print-all-cards-template.blade.php -->
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
    @php 
        $allKartuUjians = [];
        foreach($ujian->sesiUjians as $sesi) {
            foreach($sesi->kartuUjians as $kartu) {
                $allKartuUjians[] = ['sesi' => $sesi, 'kartu' => $kartu];
            }
        }
        $totalCards = count($allKartuUjians);
    @endphp

    @foreach($allKartuUjians as $index => $data)
        @php
            $sesi = $data['sesi'];
            $kartu = $data['kartu'];
        @endphp
                ...
                <div class="details">
                    <table>
                        <tr>
                            ...
                            <td>: {{ 'MRT-' . str_pad($kartu->siswa->id, 4, '0', STR_PAD_LEFT) }}</td>
                        </tr>
                        <tr>
                            <td class="label">Nama</td>
                            <td>: {{ strtoupper($kartu->siswa->name) }}</td>
                        </tr>
                        <tr>
                            <td class="label">Kelas</td>
                            <td>: {{ $kartu->siswa->class }}</td>
                        </tr>
                        <tr>
                            <td class="label">Username</td>
                            <td>: {{ $kartu->siswa->nis }}</td>
                        </tr>
                        <tr>
                            <td class="label">Password</td>
                            <td>: Merdeka{{ substr($kartu->siswa->nis, -4) }}</td>
                        </tr>
                        <tr>
                            <td class="label">Ruang</td>
                            <td>: {{ $sesi->ruang->name }}</td>
                        </tr>
                    </table>
                </div>
            </div>
